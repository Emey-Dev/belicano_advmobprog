...
class Cart {
  ...
  Cart copyWithProducts(List<CartProduct> newProducts) {
    ...
    final double newDiscountedTotal =
        newProducts.fold(0.0, (sum, p) => sum + p.discountedTotal);
    return Cart(
      id: id,
      ...
      totalQuantity: newProducts.fold(0, (sum, p) => sum + p.quantity),
      isDeleted: isDeleted,
      deletedOn: deletedOn,
    );
  }
...
class CartProductInput {
  ...
  const CartProductInput({required this.id, required this.quantity});
